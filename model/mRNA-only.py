...
def cv_validation():
    ...
    for i in range(5):
        ...
        fcn = fcn_model()
        plot_model(fcn, show_shapes = True, to_file = '/media/user/Disk 02/wangzhiqin/TensorMulti/result/mRNA-only/fcn_model.png')
    
        fcn.summary()
       
        log_filepath = '/media/user/Disk 02/wangzhiqin/TensorMulti/result/mRNA-only/model_log/' + 'cv_' +str(i)
...
#         loss_function = binary_focal_loss(gamma=2, alpha=0.25)
        ...
        history = fcn.fit(x_train_gene, y_train_gene_onehot, epochs=11, batch_size=30, validation_split = 0.2, callbacks=[tb_cb])
        fcn.save_weights('/media/user/Disk 02/wangzhiqin/TensorMulti/result/mRNA-only/save_model/' + 'cv_' +str(i) + '_weight.h5')
...
#         plt.savefig('/media/user/Disk 02/wangzhiqin/TensorMulti/result/mRNA-only/' + 'cv_'+str(i)+'acc.png')
...
#         plt.xlabel('Epoch')
#         plt.legend(['Train', 'Validation'], loc='upper left')
#         plt.savefig('/media/user/Disk 02/wangzhiqin/TensorMulti/result/mRNA-only/'+ 'cv_'+str(i)+'loss.png')

    # save for KM
    np.save('/media/user/Disk 02/wangzhiqin/TensorMulti/result/mRNA-only/km/mRNA_only_predict_label.npy',predict)
    np.save('/media/user/Disk 02/wangzhiqin/TensorMulti/result/mRNA-only/km/mRNA_only_os_time.npy', ori_os_time)  
    np.save('/media/user/Disk 02/wangzhiqin/TensorMulti/result/mRNA-only/km/mRNA_only_state.npy', ori_os_state)
    ...
    print('\n')
    print('cindex=',cindex)
#     np.save('/media/user/Disk 02/wangzhiqin/TensorMulti/result/mRNA-only/roc/mRNA_only_ori_label.npy',ori_label)
#     np.save('/media/user/Disk 02/wangzhiqin/TensorMulti/result/mRNA-only/roc/mRNA_only_predict_score.npy', predict_score)
